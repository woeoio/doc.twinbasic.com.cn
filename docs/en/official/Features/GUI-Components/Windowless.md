...
1. **Render the Entire Form to a Bitmap**
   ...
   * In TwinBASIC: Use the form’s [`Canvas`](/en/official/Reference/CustomControls/Framework/Canvas) or [`ICustomControl.Paint`](/en/official/Reference/CustomControls/Framework/ICustomControl#paint) logic to manually render windowless elements to a bitmap. See the [CustomControls package reference](/en/official/Reference/CustomControls/) for the full framework API.

2. **Ensure Windowless Controls Are Painted**

   * For [custom controls](/en/official/Reference/CustomControls/) using [`ICustomControl.Paint`](/en/official/Reference/CustomControls/Framework/ICustomControl#paint), call their paint routines manually into the same bitmap or `DC`.
   * If using [`Canvas.RuntimeUICCCanvasAddElement`](/en/official/Reference/CustomControls/Framework/Canvas#runtimeuicccanvasaddelement), simulate a paint pass with the same layout logic used during runtime.
