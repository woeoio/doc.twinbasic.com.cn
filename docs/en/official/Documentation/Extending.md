...
If the stage writes new fields to page objects, add them at the point in `runBuild` where they first appear, and list them in the [data model table](/en/official/Documentation/Pipeline-Stages#page-objects-pages) in Pipeline Stages so other developers know which phase sets each field.
...
- [Pipeline Stages](/en/official/Documentation/Pipeline-Stages) -- full data model and export reference for every stage.
