...
_outputlist_
: _optional_ One or more comma-delimited numeric expressions or string expressions to write to a file.

Data written with **Write #** is usually read from a file with [**Input #**](/en/official/Reference/Core/Input).

When _outputlist_ is omitted and a comma follows _filenumber_, a blank line is printed to the file. Multiple expressions can be separated with a space, a semicolon, or a comma. A space has the same effect as a semicolon.
...
- Nothing is written to the file if _outputlist_ data is **Empty**. However, for **Null** data, `#NULL#` is written.
- For **Error** data, the output appears as `#ERROR `_errorcode_`#`. The **Error** keyword is not translated, regardless of locale.

Unlike the [**Print #**](/en/official/Reference/Core/Print) statement, the **Write #** statement inserts commas between items and quotation marks around strings as they are written to the file. Explicit delimiters in the list are not required. **Write #** inserts a newline character --- that is, a carriage return-linefeed (**Chr**(13) + **Chr**(10)) --- after it has written the final character in _outputlist_ to the file.
