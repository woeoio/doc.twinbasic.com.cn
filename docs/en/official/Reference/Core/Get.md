...
::: info
This page documents the **Get** *statement* (file I/O). The unrelated **[Property Get](/en/official/Reference/Core/Property)** procedure form is a different use of the keyword.
:::
...
Data read with **Get** is usually written to a file with [**Put**](/en/official/Reference/Core/Put). The first record or byte in a file is at position 1, the second record or byte is at position 2, and so on. When *recnumber* is omitted, the next record or byte following the last **Get** or **Put** statement (or pointed to by the last [**Seek**](/en/official/Reference/VBA/FileSystem/Seek) function) is read. The delimiting commas must be included:
