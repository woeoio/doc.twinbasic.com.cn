...
Specifies an interface or class that will be implemented in the [class](/en/official/Reference/Core/Class) in which it appears.

Syntax:
> **Implements** { *InterfaceName* \| *ClassName* } [ **,** { *InterfaceName* \| *ClassName* } ]…

*InterfaceName*
: The name of an interface --- either an [**Interface**](/en/official/Reference/Core/Interface) block defined in twinBASIC, or an interface in a referenced type library --- whose members will be implemented by the corresponding members in the class.
...
The **Implements** statement can't appear in a standard module --- it is valid only in a [**Class**](/en/official/Reference/Core/Class) block.

### twinBASIC enhancements

twinBASIC extends classic VBA's **Implements** in several ways. See [Inheritance](/en/official/Features/Language/Inheritance) for the full discussion; the headline differences:
...
- [Interfaces and CoClasses](/en/official/Features/Language/Interfaces-CoClasses)
