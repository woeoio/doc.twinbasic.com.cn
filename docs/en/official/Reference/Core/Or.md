...
::: info
**Or** evaluates *both* operands every time, even when *expression1* alone determines the result. Use [**OrElse**](/en/official/Reference/Core/OrElse) for short-circuit evaluation --- for example, when *expression2* is expensive, has side effects, or only matters when *expression1* is **False**.
:::
...
- [**OrElse** operator](/en/official/Reference/Core/OrElse)
