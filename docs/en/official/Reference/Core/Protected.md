...
Declares a class member (variable, procedure, or property) that is accessible from inside the declaring class and from classes that derive from it via [**Inherits**](/en/official/Features/Language/Inheritance#inherits-for-complete-oop), but not from outside callers.

::: info
The **Protected** access modifier is a twinBASIC extension over classic VBA, which has only **Public**, **Private**, and **Friend**. **Protected** is meaningful only inside an [**Inherits**](/en/official/Features/Language/Inheritance#inherits-for-complete-oop) hierarchy; in a class without inheritance, it behaves like [**Private**](/en/official/Reference/Core/Private).
:::
...
In a variable declaration, **Protected** has the same form as [**Private**](/en/official/Reference/Core/Private)/[**Public**](/en/official/Reference/Core/Public): a comma-separated list of names with optional `WithEvents`/`New` and `As`-clause. **Protected** is valid only at class scope; it cannot be used inside a procedure (use [**Dim**](/en/official/Reference/Core/Dim) or [**Static**](/en/official/Reference/Core/Static)), and it cannot be used in a [**Module**](/en/official/Reference/Core/Module) (modules have no notion of derived types).
