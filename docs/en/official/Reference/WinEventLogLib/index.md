...
The same EXE that calls [**Register**](/en/official/Reference/WinEventLogLib/EventLog#register) must be the one that calls [**LogSuccess**](/en/official/Reference/WinEventLogLib/EventLog#logsuccess) / [**LogFailure**](/en/official/Reference/WinEventLogLib/EventLog#logfailure) --- the registered **EventMessageFile** points at `App.ModulePath`, and the Event Viewer reads message strings out of that file when rendering entries.

For service / long-running classes that should expose [**LogSuccess**](/en/official/Reference/WinEventLogLib/EventLog#logsuccess) / [**LogFailure**](/en/official/Reference/WinEventLogLib/EventLog#logfailure) / [**Register**](/en/official/Reference/WinEventLogLib/EventLog#register) as if those methods were their own, see the [composition-delegation idiom](#composition-delegation-idiom) below.

## Composition-delegation idiom

A class can mix [**EventLog**](/en/official/Reference/WinEventLogLib/EventLog)`(Of T1, T2)` in through twinBASIC's [`Implements ... Via`](/en/official/Features/Language/Inheritance) composition syntax and inherit its public members unqualified:
...
The `Implements <Class> Via <field> = <expression>` clause declares a private field, evaluates the constructor expression once on first use, and forwards every public member of [**EventLog**](/en/official/Reference/WinEventLogLib/EventLog) --- [**LogSuccess**](/en/official/Reference/WinEventLogLib/EventLog#logsuccess), [**LogFailure**](/en/official/Reference/WinEventLogLib/EventLog#logfailure), and [**Register**](/en/official/Reference/WinEventLogLib/EventLog#register) --- through that field. Inside `MyService` the three methods read as if they were declared on the class itself.
...
A class can use `Implements ... Via` on [**EventLog**](/en/official/Reference/WinEventLogLib/EventLog)`(Of T1, T2)` only **once**. When several classes in the same project need to share logging, declare a single module with one event-ID enum and one category enum and `Implements ... Via` against that pair from every class. Multiple unrelated message tables are still possible --- they just have to be reached through explicitly-named [**EventLog**](/en/official/Reference/WinEventLogLib/EventLog) fields rather than the `Implements ... Via` shortcut.
...
Declare a module with two empty enum stubs, each tagged with [`[PopulateFrom]`](/en/official/Reference/Attributes#populatefrom) pointing at a project-relative JSON resource:
...
Once the JSON, the enum stubs, and the registry entries written by [**Register**](/en/official/Reference/WinEventLogLib/EventLog#register) are in place, a runtime call
...
- [EventLog](/en/official/Reference/WinEventLogLib/EventLog) -- the generic event-log source -- open / register / log entries against one event log, parameterised by an event-ID enum and a category enum
