...
# Canvas type (UDT)
The drawing surface a custom control paints onto. Passed to [**ICustomControl.Paint**](/en/official/Reference/CustomControls/Framework/ICustomControl#paint) on every redraw pass and used exclusively from inside that method --- its lifetime is the duration of the single paint pass.
...
Returns the DPI scale factor --- `RuntimeUICCGetDpi / 96` --- as a **Double**. Multiply [**PixelCount**](/en/official/Reference/CustomControls/Enumerations/PixelCount)-typed measurements by this value to convert from design-time pixels to device pixels at paint time.
