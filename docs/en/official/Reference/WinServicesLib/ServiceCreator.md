...
The generic factory the **WinServicesLib** dispatcher uses to instantiate the user's service class when the SCM launches a service. *T* is the user's [**ITbService**](/en/official/Reference/WinServicesLib/ITbService) implementation; **ServiceCreator(Of T)** wraps `New T` in a factory the dispatcher can hold by interface.

Syntax: **New ServiceCreator(Of** *T* **)**

*T*
: *required* A user class that implements [**ITbService**](/en/official/Reference/WinServicesLib/ITbService). The constraint is *practical* rather than syntactic --- there is no `Where T : ITbService` clause in the source, but the factory's `CreateInstance` returns `New T As ITbService`, which the compiler only accepts when *T* implements [**ITbService**](/en/official/Reference/WinServicesLib/ITbService).

The class is the value typically assigned to [**ServiceManager.InstanceCreator**](/en/official/Reference/WinServicesLib/ServiceManager#instancecreator):
...
Returns a fresh `New T` cast as [**ITbService**](/en/official/Reference/WinServicesLib/ITbService).

Syntax: *creator*.**CreateInstance** **As** [**ITbService**](/en/official/Reference/WinServicesLib/ITbService)
...
The method has no parameters; if the user's service class needs configuration, it should read it from the [**ServiceManager**](/en/official/Reference/WinServicesLib/ServiceManager) passed to [**EntryPoint**](/en/official/Reference/WinServicesLib/ITbService#entrypoint) rather than from constructor arguments.
...
The same indirection lets the dispatcher pair the [**ITbService**](/en/official/Reference/WinServicesLib/ITbService) instance with the [**ServiceManager**](/en/official/Reference/WinServicesLib/ServiceManager) one-to-one --- the trampoline can pass the service-specific [**ServiceManager**](/en/official/Reference/WinServicesLib/ServiceManager) into [**EntryPoint**](/en/official/Reference/WinServicesLib/ITbService#entrypoint) without the service class having to know about the manager at construction time.
