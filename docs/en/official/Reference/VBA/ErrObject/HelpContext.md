...
The **HelpContext** property is used to automatically display the Help topic specified in the [**HelpFile**](/en/official/Reference/VBA/ErrObject/HelpFile) property.

If both **HelpFile** and **HelpContext** are empty, the value of [**Number**](/en/official/Reference/VBA/ErrObject/Number) is checked. If **Number** corresponds to a built-in run-time error, the Help context ID for that error is used. If the **Number** value doesn't correspond to a built-in error, the contents page of the Help file is displayed.
