...
This example assigns the programmatic ID of an Automation object to the variable `myObjectID`, and then assigns that to the **Source** property of the **Err** object when it generates an error with the [**Raise**](/en/official/Reference/VBA/ErrObject/Raise) method.

When handling errors, do not rely on the **Source** property (or any **Err** properties other than [**Number**](/en/official/Reference/VBA/ErrObject/Number)) for control flow. The intended use of properties other than **Number** is to display detailed information to an end user when an error cannot be handled.
