...
**ReturnHResult** provides a mechanism to set an HRESULT explicitly when exiting an object method. This is particularly useful for returning non-failure (positive) HRESULTs that indicate success or status information not conveyed by the standard `S_OK` success code. Whereas [**Raise**](/en/official/Reference/VBA/ErrObject/Raise) is used to generate failure HRESULTs (negative values), **ReturnHResult** allows for setting specific non-failure results.

A calling procedure can then use [**LastHresult**](/en/official/Reference/VBA/ErrObject/LastHresult) to read the positive HRESULT that was returned from the method. Set **ReturnHResult** only when needed, typically just before the method exit point, so that the intended HRESULT is the one that's actually returned.
...
- [LastHresult](/en/official/Reference/VBA/ErrObject/LastHresult) property
