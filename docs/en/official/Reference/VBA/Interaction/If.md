...
Returns one of two values depending on a condition, evaluating only the branch it returns. **If** is a twinBASIC addition; in VBA, the closest equivalent is [**IIf**](/en/official/Reference/VBA/Interaction/IIf), which always evaluates both branches.
...
- [IIf](/en/official/Reference/VBA/Interaction/IIf) function
