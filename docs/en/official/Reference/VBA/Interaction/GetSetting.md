...
This example first uses the [**SaveSetting**](/en/official/Reference/VBA/Interaction/SaveSetting) statement to make entries in the Windows registry for the application specified as *appname*, and then uses the **GetSetting** function to display one of the settings. Because the *default* argument is specified, some value is guaranteed to be returned. Note that *section* names can't be retrieved with **GetSetting**. Finally, the [**DeleteSetting**](/en/official/Reference/VBA/Interaction/DeleteSetting) statement removes all the application's entries.
