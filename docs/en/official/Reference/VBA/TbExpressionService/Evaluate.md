...
*expression*
: *required* An object expression that evaluates to an [**ITbExpression**](./#itbexpression-interface), typically the value returned by [**Compile**](/en/official/Reference/VBA/TbExpressionService/Compile).
...
This example compiles an expression that references a property on the host object via [**AddCustomBinderObject**](/en/official/Reference/VBA/TbExpressionService/AddCustomBinderObject), then evaluates it twice with the property having different values.
