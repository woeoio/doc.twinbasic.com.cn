...
Syntax: _properties_( _DomPropertyName_ ) **As** [**HtmlElementProperty**](/en/official/Reference/tbIDE/HtmlElementProperty)

_DomPropertyName_
: _required_ The property name. **String**. Standard DOM property names, CSS-style property names (when looked up under `style`), or custom-widget property names --- all forwarded to the IDE's tool-window renderer.
