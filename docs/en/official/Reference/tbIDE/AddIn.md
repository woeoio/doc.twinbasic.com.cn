...
The class implementing **AddIn** is also the natural place to hold every other `WithEvents` reference the addin uses ([**Host**](/en/official/Reference/tbIDE/Host), each [**Button**](/en/official/Reference/tbIDE/Button), each [**ToolWindow**](/en/official/Reference/tbIDE/ToolWindow), an optional [**AddinTimer**](/en/official/Reference/tbIDE/AddinTimer), …) --- its lifetime is tied to the addin's loaded state.
