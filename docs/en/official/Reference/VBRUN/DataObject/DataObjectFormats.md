...
The result is a [**DataObjectFormat**](/en/official/Reference/VBRUN/DataObject/DataObjectFormat).

### For Each iteration

A **DataObjectFormats** object can be iterated with the [**For Each...Next**](/en/official/Reference/Core/For-Each-Next) statement, which yields each [**DataObjectFormat**](/en/official/Reference/VBRUN/DataObject/DataObjectFormat) in turn. The hidden `_NewEnum` member supplies the enumerator and is not called directly from user code.
