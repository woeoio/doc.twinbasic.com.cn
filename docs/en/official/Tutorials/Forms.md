...
1. Checks that the input is numeric before converting it. [**IsNumeric**](/Reference/VBA/Modules/Information/IsNumeric) returns `False` for empty strings, letters, or punctuation other than a decimal point or leading minus.
...
3. Calls [**Format**](/Reference/VBA/Modules/Strings/Format) to round the result to two decimal places.
...
[**vbKeyReturn**](/Reference/VBRUN/Constants/KeyCodeConstants) is the constant for the Enter key (ASCII 13). Setting `KeyAscii` to `0` tells the control not to process the keystroke further --- without this, pressing Enter in a TextBox makes a beep on most systems.
