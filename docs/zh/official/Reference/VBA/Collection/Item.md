...
*index*
: *required* An expression that specifies the position of a member of the collection. If a numeric expression, *index* must be a number from 1 to the value of the collection's [**Count**](/official/Reference/VBA/Collection/Count) property. If a string expression, *index* must correspond to the *key* argument specified when the member referred to was added to the collection.
