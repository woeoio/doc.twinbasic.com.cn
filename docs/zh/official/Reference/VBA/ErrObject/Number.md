...
When returning a user-defined error from an object, set **Err.Number** by adding the chosen error code to the [**vbObjectError**](/official/Reference/VBA/Constants/#vbObjectError) constant. For example, the following code returns 1051 as an error code:
