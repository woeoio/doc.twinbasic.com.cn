...
The class implementing **AddIn** is also the natural place to hold every other `WithEvents` reference the addin uses ([**Host**](/official/Reference/tbIDE/Host), each [**Button**](/official/Reference/tbIDE/Button), each [**ToolWindow**](/official/Reference/tbIDE/ToolWindow), an optional [**AddinTimer**](/official/Reference/tbIDE/AddinTimer), …) --- its lifetime is tied to the addin's loaded state.
