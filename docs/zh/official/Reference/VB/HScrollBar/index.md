...
An **HScrollBar** is a Win32 native horizontal scroll bar exposed as a stand-alone control. Unlike the scroll bars that automatically appear inside a [**ListBox**](/official/Reference/VB/ListBox/), [**ComboBox**](/official/Reference/VB/ComboBox/), or [**TextBox**](/official/Reference/VB/TextBox/), an **HScrollBar** is independent of any other control --- its [**Value**](#value) is whatever code reads or writes. The typical use is to control a numeric setting (a volume level, a paginator, a colour channel, the offset of a custom-drawn surface) by binding the **HScrollBar**'s [**Change**](#change) and [**Scroll**](#scroll) events to whatever the value represents.

[**VScrollBar**](/official/Reference/VB/VScrollBar/) is the vertical counterpart; the two classes are identical apart from orientation.
...
A read-only [**ControlTypeConstants**](/official/Reference/VBRUN/Constants/ControlTypeConstants) value identifying this control as a horizontal scroll bar. Always **vbHScrollBar**.
