...
For the twinBASIC-specific additions to this set --- **LongLong**, **LongPtr**, and **Decimal** as a standalone type --- see [Features → New Data Types](/official/Features/Language/Data-Types).
...
::: info
In twinBASIC, **Decimal** is available both as a **Variant** subtype (as in VBA) and as a standalone declared type --- `Dim x As Decimal` compiles and runs. The conversion function [**CDec**](/official/Reference/VBA/Conversion/CDec) returns a **Decimal** value.
:::
...
The [**Date**](/official/Reference/Core/Date) and [**Time**](/official/Reference/Core/Time) properties return the current date and time. [**Now**](/official/Reference/VBA/DateTime/Now) returns both combined. Because **Date** is ultimately a **Double**, arithmetic on **Date** values works: adding 1 advances by one day, subtracting two dates gives the number of days between them.
...
**Variant** is a tagged union that can hold any of the types in the table above, plus `Null`, `Empty`, and arrays. Its 16-byte header stores a type tag ([**VbVarType**](/official/Reference/VBA/Constants/VbVarType)) followed by type-specific data. When the value is a **String**, **Object**, or array, the 8-byte data slot holds a pointer to heap-allocated storage.

`Empty` is the default state of an uninitialised **Variant** --- it is distinct from `0`, `""`, `False`, and `Null`. Test for it with [**IsEmpty**](/official/Reference/VBA/Information/IsEmpty). `Null` propagates through arithmetic and comparison; use [**IsNull**](/official/Reference/VBA/Information/IsNull) to detect it.
...
- [New Data Types](/official/Features/Language/Data-Types) -- **LongLong**, **LongPtr**, and **Decimal** in depth
- [Enumerations](/official/Reference/Enumerations) -- index of all enumeration types across all packages
- [VbVarType](/official/Reference/VBA/Constants/VbVarType) -- **Variant** subtype tag constants
- [CDec](/official/Reference/VBA/Conversion/CDec), [CLngLng](/official/Reference/VBA/Conversion/CLngLng), [CLngPtr](/official/Reference/VBA/Conversion/CLngPtr) -- conversion functions for the three extended numeric types
