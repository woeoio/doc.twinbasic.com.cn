...
The per-instance serializer for a custom control, returned by [**CustomControlContext.GetSerializer**](/official/Reference/CustomControls/Framework/CustomControlContext#getserializer). The main entry point is [**RuntimeUISrzDeserialize**](#runtimeuisrzdeserialize) --- called from a control's [**Initialize**](/official/Reference/CustomControls/Framework/ICustomControl#initialize) to load the designer-set property values that were saved into the form's serialized data. The remaining members expose framework state --- design-mode flag, runtime / report mode, owner window handle --- that a control may need while initializing.
...
Returns **True** if the control is being created at design time (inside the form designer) rather than at run time. Controls that want to render a placeholder at design time only --- like [**WaynesTimer**](/official/Reference/CustomControls/WaynesTimer), which draws its 🕑 glyph only when **IsDesignMode** is **True** --- read this flag during **Initialize**.
...
Returns **True** if the control was added at run time (via [**CustomControlsCollection.Add**](/official/Reference/CustomControls/Framework/CustomControlsCollection#add)) rather than placed in the form designer.
