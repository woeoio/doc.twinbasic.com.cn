...
---
# PixelCount
A **Long**-compatible type alias used wherever a measurement in pixels is expected. Used pervasively across the package --- every control's inherited **Left**, **Top**, **Width**, **Height** are typed as **PixelCount**, as are [**Corner.Radius**](/official/Reference/CustomControls/Styles/Corners#radius), [**Padding**](/official/Reference/CustomControls/Styles/Padding) fields, [**Border.StrokeSize**](/official/Reference/CustomControls/Styles/Borders#strokesize), and many more.
...
The package treats one **PixelCount** as one *unscaled* pixel; the design-time canvas is at 96 DPI. The runtime scales the value for the active monitor's DPI when painting, so a 15-pixel corner radius looks the same on a 96-DPI and a 192-DPI display. Inside an [**ICustomControl.Paint**](/official/Reference/CustomControls/Framework/ICustomControl#paint) implementation, the scale factor is available through [**Canvas.RuntimeUICCGetDpiScaleFactor**](/official/Reference/CustomControls/Framework/Canvas#runtimeuiccgetdpiscalefactor).
