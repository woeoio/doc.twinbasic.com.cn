...
A **ParentControls** object can be iterated with the [**For Each...Next**](/official/Reference/Core/For-Each-Next) statement, which yields each sibling control in turn, in the order the host returns them. The hidden `_NewEnum` member supplies the enumerator and is not called directly from user code.
