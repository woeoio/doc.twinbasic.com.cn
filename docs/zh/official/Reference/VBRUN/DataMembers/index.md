...
A **DataMembers** object can be iterated with the [**For Each...Next**](/official/Reference/Core/For-Each-Next) statement, which yields each name in turn, in the order it was added. The hidden `_NewEnum` member supplies the enumerator and is not called directly from user code.
