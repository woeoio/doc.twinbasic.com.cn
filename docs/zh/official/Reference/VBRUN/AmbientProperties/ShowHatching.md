...
Selection hatching is the diagonal cross-hatch the IDE draws over an inactive embedded object to make clear it is selected but not yet activated for editing. A control that paints its own selection feedback should overlay a hatching pattern when this property is **True**. As with [**ShowGrabHandles**](/official/Reference/VBRUN/AmbientProperties/ShowGrabHandles), the property is generally only meaningful while [**UserMode**](/official/Reference/VBRUN/AmbientProperties/UserMode) is **False**.
