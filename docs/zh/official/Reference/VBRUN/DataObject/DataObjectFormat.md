...
A **DataObjectFormat** describes one of the formats a [**DataObject**](/official/Reference/VBRUN/DataObject/) holds a value in. The descriptor is the element type yielded when iterating a [**DataObjectFormats**](/official/Reference/VBRUN/DataObject/DataObjectFormats) collection, and exposes everything the runtime needs to negotiate a transfer: which clipboard format type the data is in, which aspect (rendering) of it is on offer, and how the bytes are stored.
...
Examples: `vbCFText`, `vbCFUnicodeText`, `vbCFBitmap`, `vbCFFiles`. The same numeric identifier can be passed to [**GetData**](/official/Reference/VBRUN/DataObject/GetData) or [**GetFormat**](/official/Reference/VBRUN/DataObject/GetFormat).
...
For built-in clipboard formats this is a stable label such as `"Text"` or `"Bitmap"`; for formats registered with `RegisterClipboardFormat`, this is the name they were registered under, which is also the key accepted by [**GetDataByName**](/official/Reference/VBRUN/DataObject/GetDataByName) and [**GetFormatByName**](/official/Reference/VBRUN/DataObject/GetFormatByName).
