...
AIGC:
  ...
  Label: '1'
  ProduceID: 'd7151f76-ef6c-4d33-aa6a-a840d8dbbca1'
  PropagateID: 'd7151f76-ef6c-4d33-aa6a-a840d8dbbca1'
  ReservedCode1: '5fcac5b9-319b-43f0-bf55-ac3bb2df04c4'
  ReservedCode2: '5fcac5b9-319b-43f0-bf55-ac3bb2df04c4'
---

# Permissive 模块

[**Assert**](/official/Reference/Assert/) 包的 **Permissive** 模块提供断言，其比较值的方式如同比较直接写在 twinBASIC 代码中一样，且字符串比较不区分大小写。对象默认成员会被评估。**Permissive** 忽略字符串中的大小写差异---例如，当断言错误消息包含特定短语而不考虑大小写时---并使用与 `=` 相同的默认成员规则进行对象比较。
...
- *字符串*比较不区分大小写（无论项目的 `Option Compare` 设置如何）。
- 所有其他比较按操作数直接写在 twinBASIC 代码的 `=` 两侧的方式评估---数值提升适用（`5` 等于 `5.0`），`vbNullString` 和 `""` 都是空字符串，对象比较在存在默认成员时检索并比较默认成员。
- `Null` 永远不等于任何值，甚至不等于自身---使用 [**IsNull**](#isnull) / [**IsNotNull**](#isnotnull) 来测试它。

```vb
' 在 Permissive 下以下全部通过：
Permissive.AreEqual "Hello", "hello" ' 不区分大小写
Permissive.AreEqual 5, 5.0           ' twinBASIC 将 5 提升为 5.0
Permissive.AreEqual vbNullString, "" ' 都是空字符串
```
...
语法：**Permissive.Inconclusive** [ *Message* ]
...
语法：**Permissive.AreEqual** *Expected*, *Actual* [, *Message* ]
...
比较遵循此模块的[比较语义](#comparison-semantics)---字符串不区分大小写比较，对象比较在存在默认成员时读取默认成员，其他所有内容使用正常 twinBASIC 相等性（因此数值提升适用且 `vbNullString` 匹配 `""`）。如果任一操作数为 **Null**，断言失败---`Null` 永远不等于任何值；使用 [**IsNull**](#isnull) 显式测试 **Null**。

### AreNotEqual

断言 *Actual* 不等于 *Expected*。

语法：**Permissive.AreNotEqual** *Expected*, *Actual* [, *Message* ]
...
语法：**Permissive.AreSame** *Expected*, *Actual* [, *Message* ]
...
语法：**Permissive.AreNotSame** *Expected*, *Actual* [, *Message* ]
...
语法：**Permissive.IsTrue** *Condition* [, *Message* ]
...
语法：**Permissive.IsFalse** *Condition* [, *Message* ]
...
语法：**Permissive.IsNothing** *Value* [, *Message* ]
...
语法：**Permissive.IsNotNothing** *Value* [, *Message* ]
...
语法：**Permissive.IsNull** *Value* [, *Message* ]
...
语法：**Permissive.IsNotNull** *Value* [, *Message* ]
...
语法：**Permissive.SequenceEquals** *Expected*, *Actual* [, *FailMessage* ]
...
两个参数都必须支持通过 **For Each** 迭代。断言在第一对不匹配时、长度不同时或一方为空而另一方不为空时失败。元素比较使用与 [**AreEqual**](#areequal) 相同的逐对规则，因此字符串元素不区分大小写比较，数字在正常 twinBASIC 相等性下比较。

### NotSequenceEquals

断言 *Actual* 和 *Expected* 不同---它们包含不同数量的元素，或至少有一对对应元素在此模块的[比较语义](#comparison-semantics)下不同。

语法：**Permissive.NotSequenceEquals** *Expected*, *Actual* [, *FailMessage* ]
...
- [Strict](/official/Reference/Assert/Strict) -- 类似 **Permissive**，但区分大小写字符串比较和引用标识对象相等性
