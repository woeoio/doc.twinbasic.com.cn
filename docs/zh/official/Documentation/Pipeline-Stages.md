...
For design rationale and narrative descriptions, see [tbdocs Builder](/official/Documentation/Builder). To add a new stage or markdown-it plugin, see [Extending the Builder](/official/Documentation/Extending).
...
| `bookData` | `object\|null` | Parsed `_book.yml` with chapter selectors resolved to `Page` references. `null` when the file is absent. See [Book Configuration](/official/Documentation/Book-Configuration). |
...
| `createMarkdownIt` | `({ highlighter, linkTables, baseurl, staticFiles }) → MarkdownIt` | Configures and returns a markdown-it instance with all plugins and render-rule overrides applied. See [Extending the Builder](/official/Documentation/Extending) for how to add a plugin here. |
...
- [tbdocs Builder](/official/Documentation/Builder) -- architecture overview and narrative design rationale.
- [Book Configuration](/official/Documentation/Book-Configuration) -- `_book.yml` key reference.
- [Extending the Builder](/official/Documentation/Extending) -- tutorial for adding a new stage or markdown-it plugin.
