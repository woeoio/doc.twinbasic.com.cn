...
This prevents paged.js from running automatically when the bundle loads. Then it injects scripts in order via `page.addScriptTag()`:
...
2. **`PDFDocument.load(rawPdf)`** --- parses the raw PDF into pdf-lib's in-memory model. The fast-* shims (see [pdf-lib Patches](/official/Documentation/Fixes-PDFLib)) are already active from the import block; this call uses their optimised data structures.
...
- [Book Configuration](/official/Documentation/Book-Configuration) -- the `_book.yml` manifest that controls what goes into `book.html`.
- [Pipeline Stages](/official/Documentation/Pipeline-Stages) -- the `pdf.mjs` and `book.mjs` interface contracts for Phase 8.
- [tbdocs Builder](/official/Documentation/Builder) -- design rationale for Phase 8 in the tbdocs pipeline.
- [pdf-lib Patches](/official/Documentation/Fixes-PDFLib) -- detailed description of each `fast-*.mjs` shim: upstream problem, fix, and mechanism.
- [Paged.js Patches](/official/Documentation/Fixes-PagedJS) -- detailed description of every patch to `paged.browser.js`.
