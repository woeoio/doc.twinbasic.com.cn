...
1. Checks that the input is numeric before converting it. [**IsNumeric**](/official/Reference/VBA/Information/IsNumeric) returns `False` for empty strings, letters, or punctuation other than a decimal point or leading minus.
...
3. Calls [**Format**](/official/Reference/VBA/Strings/Format) to round the result to two decimal places.
...
[**vbKeyReturn**](/official/Reference/VBRUN/Constants/KeyCodeConstants) is the constant for the Enter key (ASCII 13). Setting `KeyAscii` to `0` tells the control not to process the keystroke further --- without this, pressing Enter in a TextBox makes a beep on most systems.
...
These behaviours are set through the **Anchor** and **Dock** properties on the VB package controls. See [Features → Anchoring and Docking](/official/Features/GUI-Components/Anchoring-Docking) for a full explanation.

## Where to go next

- **Windows API** -- calling Win32 functions to read system information and drive platform features: [Calling the Windows API](/official/Tutorials/Windows-API)
- **Custom controls** -- owner-drawn controls with gradient fills and per-pixel painting: [CustomControls tutorials](/official/Tutorials/CustomControls/)
- **WebView2** -- embedding the Microsoft Edge browser engine inside a form: [WebView2 tutorials](/official/Tutorials/WebView2/)
